<template>
    <div>
        <b-navbar toggleable="lg" type="dark" variant="info">
            <b-breadcrumb variant="transparent" :items="bredcrumbsItems" class="custom-breadcrumbs mb-0 bg-transparent text-light"></b-breadcrumb>
            <!-- Right aligned nav items -->
            <b-navbar-nav class="ml-auto">
                <b-nav-form>
                <b-form-input size="sm" class="mr-sm-2" placeholder="Search"></b-form-input>
                <b-button size="sm" class="my-2 my-sm-0" type="submit">Search</b-button>
                </b-nav-form>

                <b-nav-item-dropdown text="Lang" right>
                <b-dropdown-item href="#">EN</b-dropdown-item>
                <b-dropdown-item href="#">ES</b-dropdown-item>
                <b-dropdown-item href="#">RU</b-dropdown-item>
                <b-dropdown-item href="#">FA</b-dropdown-item>
                </b-nav-item-dropdown>

                <b-nav-item-dropdown right>
                <!-- Using 'button-content' slot -->
                <template #button-content>
                    <em>User</em>
                </template>
                
                <b-dropdown-item href="#">Profile</b-dropdown-item>
                <b-dropdown-item href="#">Sign Out</b-dropdown-item>
                </b-nav-item-dropdown>
            </b-navbar-nav>
        </b-navbar>
        <b-row class="m-0">
            <b-col cols="2" class="bg-light p-3">
                <b-nav vertical>                 
                    <b-nav-item to="/super_admin/statistic">Статистика</b-nav-item>
                    <b-nav-item to="/super_admin/houses">Объекты</b-nav-item>
                    <b-nav-item to="/super_admin/add_house">Добавить объект</b-nav-item>
                    <b-nav-item to="/super_admin/developer_users">Застройщики</b-nav-item>
                    <!-- <b-nav-item>Admin or no admin</b-nav-item> -->
                </b-nav>
            </b-col>
            <b-col cols="10">                
                <nuxt-child />
                <notifications group="admin-notification"/>
            </b-col>
        </b-row>
    </div>
</template>

<script>
  export default {
    data() {
      return {
        bredcrumbsItems: [
          {
            text: 'Главная',
            href: '/'
          },
          {
            text: 'Главный Администратор',
            active: true
          }
        ]
      }
    }
  }
</script>